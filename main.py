...
def woTree_task():
    ...
    try:
        ...
        for flow in flowList:
            takeFlow = client.get(
                "https://m.client.10010.com/mactivity/flowData/takeFlow.htm?flowId="
                + flow["id"]
            )
...
def daySign_task(username):
    try:
        ...
        client.get(
            "https://act.10010.com/SigninApp/signin/querySigninActivity.htm?" + param
        )
        client.headers.update(
            {
        ...
        doubleAd = client.post(
            "https://act.10010.com/SigninApp/signin/bannerAdPlayingLogo"
        )
...
def luckDraw_task():
    try:
        ...
        # 加上这一堆，看中奖率会不会高点
        client.post(
            "https://m.client.10010.com/mobileservicequery/customerService/share/defaultShare.htm"
        )
        client.get(
            "https://m.client.10010.com/dailylottery/static/doubleball/firstpage?encryptmobile="
            + numjsp
        )
        client.get(
            "https://m.client.10010.com/dailylottery/static/outdailylottery/getRandomGoodsAndInfo?areaCode=076"
        )
        client.get(
            "https://m.client.10010.com/dailylottery/static/active/findActivityInfo?areaCode=076&groupByType=&mobile="
            + numjsp
        )
        for i in range(3):
            luck = client.post(
                "https://m.client.10010.com/dailylottery/static/doubleball/choujiang?usernumberofjsp="
                + numjsp
            )
            luck.encoding = "utf-8"
            res = luck.json()
...
def gameCenterSign_Task(username):
    ...
    try:
        client.get(
            "https://img.client.10010.com/gametask/index.html?yw_code=&desmobile="
            + username
        ...
        headers = {
            ...
            "referer": "https://img.client.10010.com/gametask/index.html?yw_code=&desmobile="
            + username
            + "&version=android@8.0100",
        }
        client.headers.update(headers)
        # 进行游戏中心签到
        gameCenter = client.post(
            "https://m.client.10010.com/producGame_signin", data=data1
        )
        ...
        time.sleep(1)
        # 游戏频道积分
        gameCenter_exp = client.post(
            "https://m.client.10010.com/producGameApp", data=data2
        )
...
def openBox_task():
    ...
    try:
        # 在分类中找到宝箱并开启
        box = client.post(
            "https://m.client.10010.com/mobileService/customer/getShareRedisInfo.htm",
            data=data1,
        )
        box.encoding = "utf-8"
        time.sleep(1)
        ...
        time.sleep(1)
        # 完成任务领取100M流量
        drawReward = client.get(
            "https://m.client.10010.com/producGameTaskCenter" + param
        )
        time.sleep(1)
...
def collectFlow_task():
    ...
    try:
        for i in range(3):
            # 看视频
            watchVideo = client.post(
                "https://act.10010.com/SigninApp/mySignin/addFlow", data1
            )
...
def day100Integral_task():
    data = {"from": random.choice("123456789") + "".join(random.choice("0123456789"))}
    try:
        integral = client.post(
            "https://m.client.10010.com/welfare-mall-front/mobile/integral/gettheintegral/v1",
            data=data,
        )
...
# 位置: 发现 --> 定向积分 --> 小积分，抽好礼
def pointsLottery_task(n=0):
    try:
        numjsp = get_encryptmobile()
        # 每日首次免费
        oneFree = client.post(
            "https://m.client.10010.com/dailylottery/static/integral/choujiang?usernumberofjsp="
            + numjsp
        )
        ...
        for i in range(num):
            # 用积分兑换抽奖机会
            client.get(
                "https://m.client.10010.com/dailylottery/static/integral/duihuan?goldnumber=10&banrate=30&usernumberofjsp="
                + numjsp
            )
            # 进行抽奖
            payx = client.post(
...
# 位置: 发现 --> 定向积分 --> 每日领积分超值兑冬奥特许商品
def dongaoPoints_task():
    data = {"from": random.choice("123456789") + "".join(random.choice("0123456789"))}
    trance = [600, 300, 300, 300, 300, 300, 300]
    try:
        # 领取积分奖励
        dongaoPoint = client.post(
            "https://m.client.10010.com/welfare-mall-front/mobile/winterTwo/getIntegral/v1",
            data=data,
        )
        dongaoPoint.encoding = "utf-8"
        res1 = dongaoPoint.json()
        # 查询领了多少积分
        dongaoNum = client.post(
            "https://m.client.10010.com/welfare-mall-front/mobile/winterTwo/winterTwoShop/v1",
            data=data,
        )
        ...
        if res1["resdata"]["code"] == "0000":
            ...
            logging.info(
                "【冬奥积分活动】: " + res1["resdata"]["desc"] + "，" + str(point) + "积分"
            )
        else:
            logging.info(
                "【冬奥积分活动】: " + res1["resdata"]["desc"] + "，" + res2["resdata"]["desc"]
            )
        time.sleep(1)
    except Exception as e:
        print(traceback.format_exc())
        logging.error("【冬奥积分活动】: 错误，原因为: " + str(e))
...
def dayOneG_Task():
    try:
        ...
        res2 = getPrize.json()
        if res1["data"]["taskInfo"]["actState"] == False:
...
def getIntegral():
    try:
        integral = client.post(
            "https://m.client.10010.com/welfare-mall-front/mobile/show/bj2205/v2/Y"
        )
        integral.encoding = "utf-8"
        res = integral.json()
        for r in res["resdata"]["data"]:
            if r["name"] and r["number"]:
...
def getQuerywinning(username):
    ...
    querywinninglist = client.get(
        "http://m.client.10010.com/myPrizeForActivity/querywinninglist.htm?yw_code=&desmobile="
        + str(username)
        + "&version=android@8.0100"
    )
...
def getStorageFlow(username):
    ...
    # 获得所有流量包的标识并存储
    for li in ul:
        data = {"activeCode": None, "prizeRecordID": None, "phone": None}
...
def actionFlow(username):
    ...
    for end in endTime:
        ...
        if end < 86400:
            ...
            # 激活流量包
            res = client.post(
                "http://m.client.10010.com/myPrizeForActivity/myPrize/activationFlowPackages.htm",
                data=activeData,
            )
            res.encoding = "utf-8"
            res = res.json()
...
def check():
    ...
    # 在此之间验证是否有防刷校验
    taskCenter = client.post(
        "https://m.client.10010.com/producGameTaskCenter", data=data4
    )
...
# 拆分用户列表
def expand_users(username, password, appid):
    user_list = username.split("&")
    pswd_list = password.split("&")
    appid_list = appid.split("&")

    return list(zip(user_list, pswd_list, appid_list))


# 主函数入口
def main(username, password, appid):
    users = expand_users(username, password, appid)
    for user in users:
        # 清空上一个用户的日志记录
        open("./log.txt", mode="w", encoding="utf-8")
        global client
        client = login.login(user[0], user[1], user[2])
        if client:
            getIntegral()
            daySign_task(user[0])
            dayOneG_Task()
